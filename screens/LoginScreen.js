    ...
    <View className="bg-[#002B5B] flex-1 pt-10">
      ...
      <View className="items-center pt-10 flex-1">
        ...
        <View className="p-3 w-80 flex-row items-center justify-between rounded-lg bg-[#256D85]">
          <TextInput className="text-white px-1 flex-1" maxLength={32} value={name} placeholder="John doe " onChangeText={(val) => setName(val)} />
          {nameError && <AntDesign name="exclamationcircle" size={16} color="orange" />}
        </View>
      <TouchableOpacity className="bg-[#8FE3CF] justify-center mx-10 mt-10 w-80 p-4 rounded-lg items-center flex-row" onPress={() => handelLogin()}>
          <Text className="text-[#002B5B] text-lg">Continue 
            </Text>
            <MaterialCommunityIcons name="chevron-right" size={24} color="8FE3CF"/>
      </TouchableOpacity>
      </View>
    </View>
  )
